...
Vec::Vec(REAL x_, REAL y_) {
    x = x_;
    y = y_;
    z = 0;
}
Vec::Vec(REAL x_, REAL y_, REAL z_) {
    x = x_;
    y = y_, z = z_;
}
Vec::Vec(const Vec& v) {
...
REAL Vec::LenSqr() { return SQR(x) + SQR(y) + SQR(z); }

REAL Vec::Len() { return sqrt(LenSqr()); }
...
Vec Vec::GeometricMedian(std::vector<Vec>& points)  {
    ...
    for (unsigned int i = 0; i < MAXITER; i++) {
        ...
        Vec delta = newvec - y;
        converged = true;
        if (std::abs(delta.GetX()) / y.GetX() > ITERACCURACY_REL
            and std::abs(delta.GetX()) > ITERACCURACY_ABS)
            converged = false;
        if (std::abs(delta.GetY()) / y.GetY() > ITERACCURACY_REL
            and std::abs(delta.GetY()) > ITERACCURACY_ABS)
            converged = false;
        if (std::abs(delta.GetZ()) / y.GetZ() > ITERACCURACY_REL
            and std::abs(delta.GetZ()) > ITERACCURACY_ABS)
            converged = false;
        y = newvec;
        if (converged) break;
    }
