...
double Glauber::readinrVr_for_pol_d(std::vector<double>& rVr_pro_, std::vector<double>& rVr_r_) {
    ...
    std::ifstream file("nucleusConfigurations/rVr.in"); 
    if (file.is_open()) {
        std::string line;
...
std::pair<double, double> Glauber::Psi_pm1_0(double rmag, double theta, 
    ...
    // PRL 121. 202301
    double Psi_pm1_squ =rmag * rmag * 1/16./M_PI * (4.*Ur*Ur - 2.*1.4142135623730951 * (1.-3.*cos(theta)*cos(theta)) * Ur * Vr +
                                       (5. - 3.*cos(theta)*cos(theta)) * Vr * Vr);\
    double Psi_0_squ = rmag * rmag *1/8./M_PI * (2.*Ur*Ur + 2.*1.4142135623730951 * (1.-3.*cos(theta)*cos(theta)) * Ur * Vr +
