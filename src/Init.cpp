...
#pragma omp parallel for
    ...
    if (param->getUseSmoothNucleus() == 0) {
      ...
      for (int iB = 0; iB < 20001; iB++) {
          ...
          for (int i = 0; i < A1; i++) {
            for (int j = 0; j < A2; j++) {
              dx = nucleusB_.at(j).x - nucleusA_.at(i).x - impact_b + Bmag;
              dy = nucleusB_.at(j).y - nucleusA_.at(i).y;
              dij = dx * dx + dy * dy;
              p = Gtemp * exp(-Gtemp * dij / d2); // Gaussian profile
    ...
    // Shift back for the outputV_only 
    if(param->getoutputV_only() == 1 ) {
        if (param->getRotateReactionPlane()) {
            phiRP = 2 * M_PI * random->genrand64_real2();
        }
        for (unsigned int i = 0; i < nucleusA_.size(); i++) {
...
#pragma omp for
            for (int ix = 0; ix < N; ix++)  // loop over all positions
            {
                ...
                for (int iy = 0; iy < N; iy++) {
                    ...
                    lat->cells[localpos]->setTpA(0.);
                    for (int i = 0; i < A1; i++) {
                        if(param->getoutputV_only() == 1 &&  param->getoutput_spectator_V() == 1) {
                            if (nucleusA_.at(i).collided == 1) continue;
                        }
                        xm = nucleusA_.at(i).x;
                        ym = nucleusA_.at(i).y;
                    ...
                    for (int i = 0; i < A2; i++) {
                        if(param->getoutputV_only() == 1 &&  param->getoutput_spectator_V() == 1) {
                            if (nucleusB_.at(i).collided == 1) continue;
                        }
                        xm = nucleusB_.at(i).x;
                        ym = nucleusB_.at(i).y;
...
#pragma omp for
        for (int ix = 0; ix < N; ix++)  // loop over all positions
        {
            ...
            for (int iy = 0; iy < N; iy++) {
                ...
                if (param->getoutputV_only() == 1) check = 2; // Output the whole Wilson line
...
void Init::init(
    ...
    if (READFROMFILE > 0) {
        ...
        if (param->getoutputV_only() == 1) param->setSuccess(1);
